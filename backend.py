"""
    Demo app for lab 4. (Selenium Webdriver testing) 
"""
import hashlib
import os.path
...
class ApiUserSignupHandler(tornado.web.RequestHandler):
    ...
    def post(self, path):
        ...
        if user or not data['email'] or not data['display_name'] or not data['password']:
            ...
            user = self.application.add_user(data['display_name'], data['email']) 
            user.make_hash(data['password'])
        
        self.write({'token': str(uuid.uuid4()), 'user_id': user.id, 'display_name': user.display_name})
